...
final class MCUmovieCell: UITableViewCell {
    ...
    private var state: MCUmovieCellState = MovieCellStateNotRegisteredAsFavorite()
    ...
    @objc func tapTrailerButton(_ sender: UIButton) {
        let row = sender.tag
        viewModel.requestMovieTrailerData(row: row)
    }
    ...
    func setState(state: MCUmovieCellState) {
        self.state = state
    }
